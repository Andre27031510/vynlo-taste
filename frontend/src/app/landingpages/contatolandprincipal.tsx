...
@keyframes spin {
  ...
  to { transform: rotate(360deg); }
    ...
    // Dropdown é gerenciado pelo Header - não redefinir aqui
  ...
  const html = `
<section style="background: linear-gradient(135deg, #1e40af 0%, #1e293b 50%, #0f172a 100%); padding: 100px 0 80px; position: relative; overflow: hidden;">
...
`.trim();
  ...
  return (
    <>
      <Header />
      <div ref={rootRef} style={{ background: '#ffffff', paddingTop: '0px' }} dangerouslySetInnerHTML={{ __html: html }} />
      <Footer />
    </>
  );
};
